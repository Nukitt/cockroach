...
func TestGRPCInterceptors(t *testing.T) {
	...
	impl := &rpcutils.TestServerImpl{
		...
		US: func(_ *types.Any, server rpcutils.Test_UnaryStreamServer) error {
		...
		SU: func(server rpcutils.Test_StreamUnaryServer) error {
			_, err := server.Recv()
			if err != nil {
				return err
			}
		...
		SS: func(server rpcutils.Test_StreamStreamServer) error {
			_, err := server.Recv()
			if err != nil {
				return err
			}
	...
	for _, tc := range []struct {
		...
		expSpanName string
		do          func(context.Context, rpcutils.TestClient) (*types.Any, error)
	}{
		{
			name: "UnaryUnary",
			do: func(ctx context.Context, c rpcutils.TestClient) (*types.Any, error) {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rpcutils.RegisterTestServer(srv, impl)
